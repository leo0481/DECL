...
class FactorizationDataProcessor:
...
class BlendPairFactorizationDataset(FactorizationDataset):
    ...
    def make_sampler(self, flags_obj, dm):
        ...
        self.skew_sampler = SAMPLER.PairSampler(flags_obj, train_skew_lil_record, train_skew_dok_record,
                                                flags_obj.neg_sample_rate)
...
class CausEFactorizationDataset(FactorizationDataset):
    ...
    def make_sampler(self, flags_obj, dm):
        ...
        self.skew_sampler = SAMPLER.PointSampler(flags_obj, train_skew_lil_record, train_skew_dok_record,
                                                 flags_obj.neg_sample_rate)
...
class DICEFactorizationDataset(FactorizationDataset):
    ...
    def make_sampler(self, flags_obj, dm):
        ...
        self.sampler = SAMPLER.DICESampler(flags_obj, train_lil_record, train_dok_record, flags_obj.neg_sample_rate,
                                           dm.popularity, margin=flags_obj.margin, pool=flags_obj.pool)

        train_skew_coo_record = dm.skew_coo_record
        train_skew_lil_record = transformer.coo2lil(train_skew_coo_record)
        train_skew_dok_record = transformer.coo2dok(train_skew_coo_record)

        self.skew_sampler = SAMPLER.DICESampler(flags_obj, train_skew_lil_record, train_skew_dok_record,
                                                flags_obj.neg_sample_rate, dm.popularity, margin=flags_obj.margin,
                                                pool=flags_obj.pool)
...
class CGDataProcessor:
    ...
    def get_dataloader(flags_obj, test_data_source):
        ...
        return DataLoader(dataset, batch_size=flags_obj.batch_size, shuffle=True, num_workers=flags_obj.num_workers,
                          drop_last=False), dataset.max_train_interaction
...
class CGDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return torch.LongTensor([index]), torch.LongTensor(unify_train_pos), torch.LongTensor(
            unify_test_pos), torch.LongTensor([len(test_pos)])
